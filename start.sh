...
print_bold_with_outline "Welcome to Rebel V20, user!"
...
echo -e "${BLUE}[1/4] Cloning Rebel Unblocker repository...${RESET}"
if git clone https://github.com/DogeNetwork/v4 > /dev/null 2>&1; then
    echo -e "${GREEN}[Success] Repository cloned successfully.${RESET}"
else
    echo -e "${RED}[Error] Failed to clone repository. Please check your network connection, or see if the v20 folder already exists.${RESET}"
    exit 1
fi
...
if [ $? -eq 0 ]; then
    ...
    echo -e "${YELLOW}[Info] You can access Rebel V20 via localhost.${RESET}"
else
